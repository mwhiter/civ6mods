	...
	for i, section in ipairs(sections) do
		...
		if(sectionId == _CurrentSectionId) then
			...
			instance.CivilopediaSectionTabButton:RegisterCallback(Mouse.eLClick, function()
				NavigateTo(sectionId, pageId, true);
			end);
...
function RefreshHistoryButtons()
	Controls.BackPageButton:SetDisabled(CanNotClickBackButton());
	Controls.NextPageButton:SetDisabled(CanNotClickNextButton());

	if(Controls.BackPageButton:IsDisabled()) then
		Controls.BackPageButton:SetAlpha(0.4);
	else
		Controls.BackPageButton:SetAlpha(1);
	end

	if(Controls.NextPageButton:IsDisabled()) then
		Controls.NextPageButton:SetAlpha(0.4);
	else
		Controls.NextPageButton:SetAlpha(1);
	end
end

function CanNotClickBackButton()
	return (#pageVisitHistory == 0 or pageHistoryIndex == 1);
end

function CanNotClickNextButton()
...
-------------------------------------------------------------------------------
function NavigateTo(SectionId, PageId, changeHistory)
		...
		for i, page in ipairs(pages) do
			...
			if(id == PageId) then
				...
				if(changeHistory) then
					AdvancePageHistory(SectionId, PageId);
				end
			end
		end	 
	end
...
function OnOpenCivilopedia(sectionId_or_search, pageId)
	...
	if(pageId == nil and sectionId_or_search) then
		...
		if(results and #results > 0) then
			...
			NavigateTo(results[1].SectionId, results[1].PageId, true);
		else
			-- To the front page!
			local sections = GetSections();
			local pages = GetPages(sections[1].SectionId);
			local page = pages[1];
			NavigateTo(page.SectionId, page.PageId, true);
		end
...
function ClickPageBackButton()
	...
	local destPage = pageVisitHistory[pageHistoryIndex];
	NavigateTo(destPage.sectionId, destPage.pageId, false);
	RefreshHistoryButtons();
end

function ClickPageNextButton()
	if(CanNotClickNextButton()) then
		return;
	end
...
function AdvancePageHistory(SectionId, PageId)
	pageIds = { sectionId = SectionId, pageId = PageId };

	removeAllNextHistory();
	
	table.insert(pageVisitHistory, pageIds);
...
function Initialize()
	...
	-- Pagination support
	Controls.BackPageButton:RegisterCallback(Mouse.eLClick, ClickPageBackButton);
	Controls.BackPageButton:RegisterCallback( Mouse.eMouseEnter, function() UI.PlaySound("Main_Menu_Mouse_Over"); end);
	Controls.BackPageButton:LocalizeAndSetToolTip( "LOC_GREAT_PEOPLE_BACK" );

	Controls.NextPageButton:RegisterCallback(Mouse.eLClick, ClickPageNextButton);
	Controls.NextPageButton:RegisterCallback( Mouse.eMouseEnter, function() UI.PlaySound("Main_Menu_Mouse_Over"); end);
	Controls.NextPageButton:LocalizeAndSetToolTip( "LOC_GOVT_NEXT" );
		...
		if(search_term ~= nil) then
			button_control:RegisterCallback(Mouse.eLClick, function()
				...
				if(results ~= nil and #results > 0) then
					...
					NavigateTo(result.SectionId, result.PageId, true);
				end
			end);
		else
